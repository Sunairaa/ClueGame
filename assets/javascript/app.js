let mrGreen = {
  ...
  let roomsArray = [diningRoom, conservatory, kitchen, study, library, billiardRoom, lounge, ballroom, hall,
     spa, livingRoom, observatory, theater, guestHouse, patio];
  console.log(roomsArray);

  
// gets random item from each array and constructs mystery 
  ...
  // loop through all over arrays and show each elements name
  function showArrays(arr){
    arr.forEach(element => {
        document.body.innerHTML += `<li>${element.name}</li>`;        
    });
  }
  ...
  document.querySelector('#guessBtn').onclick = guess;